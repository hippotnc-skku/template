<script lang="ts">
	...
	import type { ModalPlacementType, SizeType, Dismissable } from '../types';
	...
	export let dialogClass: string =
		'fixed top-0 left-0 right-0 h-modal md:inset-0 md:h-full z-50 w-full px-2 md:p-4 flex';

	// 모달밖의 값을 리셋하고 싶을 경우 true로 하고 해당 모듈에서 on:reset써서 함수 작동시키면 된다.
	export let resetAll: boolean = false;
...
{#if open}
	...
	<div
		...
		class={twMerge(dialogClass, $$props.classDialog, ...getPlacementClasses())}
		tabindex="-1"
		aria-modal="true"
		role="dialog"
	>
		<div class="flex relative {sizes[size]} w-full max-h-full">
			...
			<Frame rounded shadow {...$$restProps} class={frameClass}>
				...
				<div
					class={twMerge(
						'px-3 md:p-6 space-y-6 flex-1 overflow-y-auto overscroll-contain',
						$$props.bodyClass
					)}
