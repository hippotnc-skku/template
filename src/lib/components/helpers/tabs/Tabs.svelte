<script context="module" lang="ts">
	...
	const styledActiveClasses = {
		...
		pill: 'py-3 px-4 text-white bg-hippNavy rounded-lg',
		underline:
			'p-4 text-blue-600 border-b-2 border-blue-600 dark:text-blue-500 dark:border-blue-500',
		none: ''
	};

	const styledInactiveClasses = {
		full: 'p-4 w-full group-first:rounded-l-lg group-last:rounded-r-lg text-gray-500 bg-white hover:text-gray-700 hover:bg-gray-50 focus:ring-4 focus:ring-blue-300 focus:outline-none',
		pill: 'py-3 px-4 text-gray-500 rounded-lg hover:text-gray-900 hover:bg-blue-200',
		underline:
...
<script lang="ts">
	...
	export let contentClass: string = 'p-4 bg-gray-50 rounded-lg mt-4';
	export let divider: boolean = true;
	export let activeClasses: string = 'p-4 text-blue-600 bg-gray-100 rounded-t-lg';
	export let inactiveClasses: string =
		'p-4 text-gray-500 rounded-t-lg hover:text-gray-600 hover:bg-gray-50';
