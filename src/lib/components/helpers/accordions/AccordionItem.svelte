<script lang="ts">
	import ChevronDown from '$lib/components/helpers/utils/ChevronDown.svelte';
	import ChevronUp from '$lib/components/helpers/utils/ChevronUp.svelte';
	...
	export let defaultClass: string =
		'flex items-center justify-between w-full font-semibold text-left transition-all border-slate-100 ease-soft-in text-slate-500';
	...
	const ctx: AccordionCtxType = getContext<AccordionCtxType>('ctx') ?? {
		flush: false,
		activeClasses: '',
		inactiveClasses: ''
	};
	...
	$: buttonClass = classNames(
		...
		!open &&
			(ctx.flush ? 'text-gray-500 dark:text-gray-400' : inactiveClasses || ctx.inactiveClasses),
		$$props.class
	);
</script>
...
{#if open}
	<div transition:multiple={transitionParams}>
		<div class="{ctx.flush ? 'py-5' : 'lg:p-5'} bg-white">
			<slot />
		</div>
	</div>
{/if}
