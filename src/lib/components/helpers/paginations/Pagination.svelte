<script lang="ts">
	...
	export let activeClass: string = 'text-[#1E2666] font-bold';
	// 'text-blue-400 border border-gray-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700';
	export let normalClass: string = '';
	// 'text-gray-500 bg-white hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white';
	export let ulClass: string = 'pagination-list';
...
<nav class="pagination">
	<ul class={twMerge(ulClass, table && 'divide-x divide-gray-700', $$props.class)}>
		<li>
			<PaginationItem
				on:click={previous}
				class={twJoin(normalCls, table ? 'rounded-l' : 'rounded-l-lg')}
			>
				<slot name="prev"><i class="fa-solid fa-chevron-left" /></slot>
			</PaginationItem>
		</li>
		...
		<li>
			<PaginationItem
				on:click={next}
				class={twJoin(normalCls, table ? 'rounded-r' : 'rounded-r-lg')}
			>
				<slot name="next"><i class="fa-solid fa-chevron-right" /></slot>
			</PaginationItem>
		</li>
	</ul>
</nav>
