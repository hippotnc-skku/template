...
export function getSevenWeeksFromToday(){
	let sevenweeks = []
	for(let i = 0; i < 7; i++){
		let minusday1 = moment().tz('Asia/Seoul').subtract(i*7, 'days')
		let minusday2 = moment().tz('Asia/Seoul').subtract(i*7+6, 'days')
		sevenweeks = [((minusday2.get('month')+1)+'/'+minusday2.get('date'))+' ~ '+((minusday1.get('month')+1)+'/'+minusday1.get('date')), ...sevenweeks ]
	}
	return sevenweeks
}
