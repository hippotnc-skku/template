...
<CodeExampleFrame name="Sample">
	<div class="mx-auto max-w-[600px]">
		<HippoWhiteFrame class="px-8 py-6 mt-20">
					...
					{#if ['inserted_at', 'updated_at'].includes(key)}
						<div class="space-y-0.5">
							...
							<span class="inline-block font-semibold text-hippNavy"
								>{formatDateKr(managerData[key])}</span
							>
						</div>
					{:else}
						<div class="space-y-0.5">
							<div class="text-md font-bold">{value}</div>
							<span class="inline-block font-semibold text-hippNavy">{managerData[key]}</span>
						</div>
					{/if}
				{/each}
			</div>
...
<CodeExampleFrame name="Code">
	<iframe
		frameborder="0"
		scrolling="no"
		style="width:100%; height:1174px;"
		allow="clipboard-write"
		src="https://emgithub.com/iframe.html?target=https%3A%2F%2Fgithub.com%2Fhippotnc-skku%2Ftemplate%2Fblob%2Fcomponent%2Fsrc%2Froutes%2F%28template%29%2Fcru%2Fdetail-readtwo%2F_temp.svelte%3Fts%3D4%23L1-L54&style=a11y-dark&type=code&showBorder=on&showLineNumbers=on&showFullPath=on&showCopy=on"
	></iframe>
</CodeExampleFrame>
